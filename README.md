...
- **横幅提醒**：当某个编号的某个进球数连续超过警戒线（默认 **30 天**，可在 `config.json` 的 `alertDays` 修改）没有出现时，网页顶部会弹出横幅提醒（哪个编号、哪个进球数、多少天没出、最近一次是哪天），点「查看全部」直达追踪列表。条件持续存在时横幅每次打开都会提醒；运行日志与油猴浮窗也会有提示
- **手动查询**：网页「统计分布」页的「编号追踪」卡片里，选任意 编号 × 进球数（如 005 的 4球），即可查看：距今多少天没出现、连续未出现多少次、历史出现次数、最近一次是哪天；下面还列出该编号全部 8 个进球数的完整间隔表
- **追踪列表**：所有仍活跃编号的 编号×进球数 组合按"距今天数"排序，超警戒（红）、接近警戒（橙）、正常一目了然；可勾选「只看达到警戒线的」
- **Excel**：新增「编号统计」（每个编号的进球数分布）和「编号追踪」（间隔矩阵 + 警戒列表）两个工作表
...
口径说明：编号 = 场次编号的数字部分（如 周日005 → 005）；「距今」按日历天计（到最近有数据的一天）；「连续未出现」按该编号实际出现的次数计；只对近期仍活跃（近 7 天出现过）的编号、且历史上出现过至少一次的分档进入警戒列表。
