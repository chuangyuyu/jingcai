...
口径说明：编号 = 场次编号的数字部分（如 周日005 → 005），按**销售日**归属——周三的场次包含其凌晨开赛的比赛（真实开赛日在周四）；不同销售日的同号场次（如 周三004 与 周二004 都开赛于周三）是两场比赛，分别统计、不会混淆。「连续未出现」按该编号实际踢过的场次计（没有比赛的日子不计入）；只对近期仍活跃（近 7 天出现过）的编号、且历史上出现过至少一次的分档进入警戒列表。
